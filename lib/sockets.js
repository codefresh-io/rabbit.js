...
inherits(WorkerSocket, EventEmitter);
        ...
        .then(function(ok) {
          ...
          return ch.consume(ok.queue, function(msg) {
                if (msg !== null) {
                  ...
                  ch.ack(msg); //TODO maybe move this to be after invoking callback (or even better once finished)
                }
                //else self.push(null);
...
inherits(ReqSocket, EventEmitter);
...
ReqSocket.prototype.handleReply = function(msg) {
  ...
  for (var i = 0; i < this.awaiting.length; i++) {
    if (cid === this.awaiting[i].correlationId) {
      var awaiting = this.awaiting.splice(i, 1)[0];
      awaiting.callback(msg);
    }
  }
};
...
ReqSocket.prototype.write = function(chunk, encoding, callback) {
  ...
  if (queue) {
    this.queues.push(queue);
    var corrId = guid();
    this.awaiting.push({correlationId: corrId, answer: null, callback: callback}); //TODO support promises
...
inherits(RepSocket, EventEmitter);
...
RepSocket.prototype.write = function(msg, chunk, encoding) {
  var ch = this.ch, current = msg;
...
RepSocket.prototype.requeue = function(msg) {
    var ch = this.ch, current = msg;
...
RepSocket.prototype.discard = function(msg) {
    var ch = this.ch, current = msg;
